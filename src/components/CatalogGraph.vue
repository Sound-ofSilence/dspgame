...
function renderChart() {
  ...
  // 构建节点：使用 Category 来区分颜色和大小
  const nodes = Object.values(recipeStore.items)
    .filter(i => nodeIds.has(i.ID))
    .map(item => {
      // 防御性判断：优先读 Category 字段，兼容旧数据
      const isBuilding = item.Category === 'building' || (item.Category === undefined && item.Type === 4);
      return {
        id: String(item.ID),
        name: item.Name,
        symbolSize: isBuilding ? 35 : 20,
        itemStyle: { color: isBuilding ? '#e6a23c' : '#409eff' }
      };
    });
  ...
  chartInstance.setOption({
    title: { 
      text: `物品合成关系网 (当前展示 ${nodes.length} 个节点)`, 
      left: 'center' 
    },
    tooltip: { 
...
onMounted(() => {
  // 初始化选项：全部物品和建筑
  allOptions.value = Object.values(recipeStore.items).map(i => ({ 
    value: i.ID, 
    label: i.Name 
  }));
  initChart();
...
<template>
  <div class="graph-wrapper">
    <div class="graph-toolbar">
      ...
      <span class="hint">提示：默认展示前50个节点，可搜索或从下拉框选择特定节点精确渲染。</span>
    </div>
